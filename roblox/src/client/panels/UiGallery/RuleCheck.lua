...
function RuleCheck.run(gallery)
	...
	-- ⑭ 토스트: TC 1행 · 대기열 8 · 낮은 priority부터 버림(TC · BC 규칙) / TR 줄 수(남은 자리 1 ~ 3 - FeedLayout) · 새 알림이 위 · 오래된 줄이 밀림(S10 - 대기열 없음) · 묶기
	Toast.clear()
	for index = 1, 5 do
	...
	local feedExpected = table.concat(table.move({ "항목 5", "항목 4", "항목 3" }, 1, math.min(feed.capacity, 3), 1, {}), " · ")
	Toast.clear()
	...
	Toast.clear()
	check(("⑭ 토스트 TC: 5개 → 보이는 행 %d(기대 1) · 대기 %d(기대 4) / 16개 → 대기 %d(기대 8, 넘치면 낮은 priority부터 버림) · TR: 5개 → 행 %d(기대 줄 수 %d) · 대기 %d(기대 0) · 밀려난 %d(기대 %d) · 위에서부터 [%s](기대 %s) · 같은 groupKey 두 번 → %s · 행 %d(기대 merged · 1)"):format(
		afterFive.rows, afterFive.queued, afterMany.queued, feed.rows, feed.capacity, feed.queued, feed.evicted, 5 - feed.capacity, feedTexts, feedExpected, merged, afterGroup.rows),
		afterFive.rows == 1 and afterFive.queued == 4 and afterMany.queued == Toast.queueMax
			and feed.capacity >= 1 and feed.rows == feed.capacity and feed.queued == 0 and feed.evicted == 5 - feed.capacity and feedTexts == feedExpected
