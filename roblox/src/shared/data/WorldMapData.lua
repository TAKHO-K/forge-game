...
return {
	...
	hub = {
		...
		tree = {
			...
			course = {
				...
				bounce = { reachStuds = 22, horizontalMax = 14 }, -- 통통 열매: 밟으면 발 +reach(클라 속도) - 공중 점프는 그 뒤에도 쓴다(서버 높이 허가 - LaunchPermitRequest · M1-2c)
				...
				lift = { angleDeg = 0, r = 70, label = "덩굴 리프트", vineDia = 2.4, leafEvery = 10, basketRadius = 5, promptDistance = 10, promptSlackStuds = 2, rideCooldownSeconds = 1.5, signHeight = 11,
	...
	travel = {
		...
		-- replicateWaitSeconds = 서버가 옮긴 자리가 내 캐릭터에 반영되기를 기다리는 상한 · fallSkipAfterTeleportSeconds = 순간이동 직후 나무 "떨어짐" 판정을 건너뛰는 시간(옛 위치 복제)
		arrival = { probeStuds = 40, holdMaxSeconds = 3, nearStuds = 12, replicateWaitSeconds = 1, fallSkipAfterTeleportSeconds = 1 },
	},
