...
return {
	...
	-- P2.5a C10: 0.035 → 0.025. 스테이지가 약 7배 촘촘해져(k 1.155 → 1.02) 같은 성장이 log2 축에서 더 길어진다 - 옛 곡선의 전체 성장폭 f(4738) ÷ f(125) = 1.184(옛 SafeStageCap까지)를
	-- 새 설계 최대(약 21,000)까지 같게 옮긴 값: 1 + p × log2(21000 ÷ 125) = 1.184 → p = 0.0249.
	levelLogSlope = 0.025,
	-- P2.5c 결정 5: 옵션(보석) 등급 몫의 등급당 배율(Option.gradeFactor - 장비는 ItemVisualData.gradeStep 1.45 그대로). 일반 등급 기준점은 장비와 같다.
	-- 값 = 일반 프로필 보석 위력 합이 50 ~ 60%(스테이지 500 ~ 20,000)가 되게 하네스로 고른 값(docs/phase/P25c-log.md).
	gradeStep = 1.35,
	rollMin = 0.875,
	rollMax = 1.125,
