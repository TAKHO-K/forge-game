...
return {
	...
	classes = {
		...
		bow = {
			...
			-- 1.55배로 "직업 간 단일 총딜 1.32배 이내"(PRD-forge-game-roblox.md 20.36 [4])를
			-- 넘고, 쌍검(812.4)보다도 높아 "딜 천장은 쌍검"(PRD-forge-game.md 4.4) 서열까지
			-- 뒤집었다. 속사·꽂힌 화살·백스텝샷이 전부 평타 횟수에 비례하므로 공속 하나로
			-- 전체가 같은 비율로 내려간다. 지시의 0.85(C/B 적용 전 추정)는 재측정에서 797.5 =
			-- 1.332배로 1% 넘겨 0.84로 미세 조정했다(≈788, 1.317배, 쌍검 812.4 아래). 20.44 [1]-A 참고.
			atkSpeed = 0.84,
			def = 0.6,
			critRate = 0.15,
			critDmg = 2.3,
