...
return {
	...
	-- 첫 환생 전(rebirthCount==0) 경험치 배수(23-3, 지시 "초반 레벨업이 지루하다, 첫
	-- 환생까지는 10마리 안팎으로 1업"). 새 상수를 임의로 고른 게 아니라 위 weaponLevelExp
	-- 표(1~25) + monsterExpCoefficient로 실제 계산한 "레벨당 처치 수"의 현재 평균(24개
	-- 구간 합계 1480.9마리, 평균 61.7마리/레벨 - PowerShell로 역산)을 10마리/레벨 평균이
	-- 되도록 나눈 값이다: 1480.9 / (10×24) = 6.17. 26+ 구간(레벨당 25마리 고정)과 달리
	-- 1~25 구간은 표 자체가 손튠이라 레벨마다 배수 적용 후에도 4~14마리 사이로 흩어진다
	-- (표의 원래 형태를 유지한 채 배수만 걸어서다) - "정확히 10"이 아니라 "10마리 안팎"이라는
	-- 지시 표현과 맞춘 것이다. 정확한 레벨별 표는 보고서 "임의 결정" 목록 참고.
	firstRunExpMultiplier = 6.17,
}
