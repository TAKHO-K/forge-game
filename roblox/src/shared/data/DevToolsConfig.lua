...
return {
	...
	verify = {
		...
		-- S13(2026-09-20): 이번 세션 블록 S13(가)(앵커 DPS · 1.32 규칙 · p · b 불변 · 딜링모드 가동률 · 옵션 색 데이터) · S13(나)(실제 HealCast 경로 파티 회복) +
		-- 동반 실행(COMMON.md §3): BalanceSim(회복률 인자 추가)을 쓰는 27-1(가)(딜링모드 가동률 세 줄 + 보스전 두 줄 추가) · 26-2(생존 타수 measurePoint) · S09(가)(p · b 불변) ·
		-- SkillData(대검 계수 · 치유 플래그)를 읽는 S08(가) · ItemDescribe · ItemTooltip(옵션 색 필드)을 쓰는 S12b(가)(표시 형식) · S12b(UI)(알림 툴팁 · 창 글씨) ·
		-- InventoryUI(옵션 줄 글씨색)와 창 크기 점검 S12(UI). S04(나)는 계속 제외.
		current = { "S13(가)", "S13(나)", "27-1(가)", "26-2", "S09(가)", "S08(가)", "S12b(가)", "S12b(UI)", "S12(UI)" },
	},
}
