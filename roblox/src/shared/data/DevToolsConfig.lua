...
return {
	...
	verify = {
		...
		-- S12(2026-09-20): 이번 세션 블록 S12(나)(견습 중 파티 · 파티 보스 견습 멤버 제외 · 견습 보스 싱글 · 보상 기준 + 기존 party selftest 11항목) + 클라 자체 점검 S12(UI)(사전 작업 2 · 3 - 접속 10초 뒤, 서버 체인과 무관) +
		-- 동반 실행(COMMON.md §3): 이 세션이 고친 BossEncounter(getEntryMembers · checkPartyEntry · spawnForParty) · PartyVote(대상 = 입장 멤버) · TutorialState(start의 파티 탈퇴 제거)를 쓰는 옛 블록 -
		-- 27-3(가)(나)(PartyVote 투표) · S09(나)(스탠드인 파티 · 처치 경험치) · S10(나)(스탠드인 파티 · 견습 확정 지급) · 29-5(나)(더미 파티 보스 입장 spawnForParty).
		current = { "S12(나)", "S12(UI)", "27-3(가)", "27-3(나)", "S09(나)", "S10(나)", "29-5(나)" },
	},
}
