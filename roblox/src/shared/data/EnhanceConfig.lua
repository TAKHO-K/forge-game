...
local config = {
	...
	goldCost = {
		...
		-- P2.5a C5: 21 ~ 30강 다시 설계 - +20→21(766,000)부터 1회 비용 ×1.1씩(유효숫자 세 자리 · 1,000 단위). 옛 21 ~ 25강(3.48e6 · 1.59e7 · 8.42e7 · 6.24e8)은
		-- 새 k의 골드 수입(C8 - 구매력 +20→21 기준 일반 분당 0.0012회)으로 +24 이후를 누구도 못 닿게 했다. 목표 = 상위 1%(방지권 사용)가 약 2,190시간에 +30,
		-- 일반이 +24(목표 단계)에 약 600시간 - 0 → 30 기대 골드 3.2e8(방지권 없이, 몬테카를로 3,000회 - docs/phase/P25a-log.md).
		766000, 843000, 927000, 1020000, 1122000,   -- +20~+24 -> +21~+25
		1234000, 1357000, 1493000, 1642000, 1806000,   -- +25~+29 -> +26~+30
	},
}
