...
function M1Verify.runLive(player, env)
	...
	r.section("몬스터 지대", function()
		local z1 = WorldMapLayout.zoneByKey("tier1")
		local g = WorldMapLayout.grounds(z1)[2]
		put(g.center + Vector3.new(0, 6, 200))
		task.wait(2.2)
		...
		r.check(("사냥 지대 곁 → 켜진 지대 %d · 몬스터 %d / 모두 떠나 %d초 → 켜진 지대 %d · 몬스터 %d"):format(a1, alive1, D.spawnSites.idleSeconds, a2, alive2), a1 >= 1 and alive1 >= 9 and a2 == 0 and alive2 == 0)
	end)
