...
function GemFlowVerify.runLive(player, env)
	...
	local ok1, why1 = GemEquip.handle(player, 2, 1, far)
	t.check("강화대에서 멀면 far + 상태 불변", ok1 == false and why1 == "far" and fingerprint(player) == seededPrint)
	local ok2, why2 = GemEquip.handle(player, "x", 1, near)
	t.check("인자 모양이 틀리면 invalid", ok2 == false and why2 == "invalid")
	local ok3, why3 = GemEquip.handle(player, 5, 1, near)
	t.check("잠긴 홈 -> slot_locked + 상태 불변", ok3 == false and why3 == "slot_locked" and fingerprint(player) == seededPrint)
	local ok4, why4 = GemEquip.handle(player, 2, 99, near)
	t.check("없는 보석 -> not_found + 상태 불변", ok4 == false and why4 == "not_found" and fingerprint(player) == seededPrint)
	local ok5, why5 = GemEquip.handle(player, 2, 2, near)
	...
	local ok6, why6 = GemEquip.handle(player, 2, 1, near)
	local last = inventory[#inventory]
	t.check("정상 장착 -> 성공 + 이유 없음", ok6 == true and why6 == nil)
	...
	for slot = 1, Gem.slotCount do
		for index = 1, #SEED_INVENTORY do
			...
			local success, reason = GemEquip.handle(player, slot, index, near)
			combos += 1
	...
	for index = 1, #SEED_INVENTORY do
		...
		if slot then
			autoCount += 1
			local success = GemEquip.handle(player, slot, index, near)
			if not success then
				autoBad += 1
			end
		end
