...
function BossArenaContainment.checkMember(encounter, member)
	...
	-- M1-0: 공중 점프로 벽 윗면 높이를 1초 넘게 날 수 있다(단상 위 연속 공중 점프) - 시간은 벽 위에 "서 있을" 때 시작한다. 벽 밖으로 날아가면 위의 isOutside가 받는다.
	-- 리뷰 2: 한 번 서면 발이 그 높이 아래로 내려갈 때까지 계속 잰다(벽 위에서 점프를 누르고 있어 폴링이 공중만 봐도 지우지 않는다).
	local standingOn = humanoid ~= nil and humanoid.FloorMaterial ~= Enum.Material.Air
	if not lifted and (standingOn or offFloorSince[member]) and ArenaContainment.isOffFloorHeight(root.Position.Y - standing, floorTopY, GEOMETRY.wallHeightStuds) then
