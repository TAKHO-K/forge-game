...
-- ═══ P2.5b 자동 검증 블록(가) - 계승 · 재련 · 분해 · 마일스톤 규칙(순수 계산) ═══
if RunService:IsStudio() and verifyEnabled("P25b(가)") then
	task.spawn(function()
...
-- ═══ P3d 자동 검증 블록(가) - 파티 구성 속도 · 툴팁 · 견습 시뮬 · 첫 시즌 KST · 복귀 시뮬 · 재생성 회피 부등식 · 연출 데이터(docs/phase/P3d-log.md) ═══
if RunService:IsStudio() and verifyEnabled("P3d(가)") then
	task.spawn(function()
		local ok, err = pcall(require(script.Parent.P3dVerify).runPure)
		if not ok then
			warn(("[P3d(가)] 검증 블록 에러: %s"):format(tostring(err)))
		end
	end)
end
...
-- ═══ G1-3 자동 검증 블록(가) - 레벨차 계수 · 환생 자리(docs/phase/G1-3-report.md) ═══
if RunService:IsStudio() and verifyEnabled("G1-3(가)") then
	task.spawn(function()
...
-- ═══ M1-4 자동 검증 블록(가) - 소품 라이브러리 · 빈 공간 · 커브길 · 무리 스폰 · 외곽 테마 · 관문 틀 · 심해 신전 · 나무(docs/phase/M1-4-report.md) ═══
if RunService:IsStudio() and verifyEnabled("M1-4(가)") then
	task.spawn(function()
		local ok, err = pcall(require(script.Parent.M1_4Verify).runPure)
		if not ok then
			warn(("[M1-4(가)] 검증 블록 에러: %s"):format(tostring(err)))
		end
	end)
end
...
-- ═══ M1-3 자동 검증 블록(가) - 관문 목록 · 파티 규칙 · 저장 v40 · 지형(docs/phase/M1-3-report.md) ═══
if RunService:IsStudio() and verifyEnabled("M1-3(가)") then
	task.spawn(function()
		local ok, err = pcall(require(script.Parent.M1_3Verify).runPure)
		if not ok then
			warn(("[M1-3(가)] 검증 블록 에러: %s"):format(tostring(err)))
		end
	end)
end
...
-- ═══ G1-2 자동 검증 블록(가) - 드랍 공정성 보정 · 분해 문턱 · 저장 v36(docs/phase/G1-2-report.md) ═══
if RunService:IsStudio() and verifyEnabled("G1-2(가)") then
	task.spawn(function()
		local ok, err = pcall(require(script.Parent.G1_2Verify).runPure)
		if not ok then
			warn(("[G1-2(가)] 검증 블록 에러: %s"):format(tostring(err)))
		end
	end)
end
