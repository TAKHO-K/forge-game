...
> 마지막 갱신: **M1-2c 점프대 × 높이 검사 · 덩굴 리프트 · 2026-09-26** · 직전 보고서 = `docs/phase/M1-2c-report.md`(그 전 `M1-3a-report.md`) · 스트리밍 = `docs/perf/streaming-settings.md` · 설계 = `docs/design/world-map-m1.md`(세계 지도 · 좌표) · 보스 = `docs/design/boss-br1-2.md` · 패턴 설명 = `docs/design/boss-patterns-explained.md`
...
**진행 중 = M1-3 본편**(재개: `docs/phase/M1-3-log.md` "진행 지점" 표의 마지막 [ ] 줄부터 - 코드 대부분 커밋됨 · 남은 것 = 전 구역 굽기 · 검증 블록 M1-3T · 성능 · 스크린샷 · 보고서). 원래 계획: **M1-3 본편**(대형 지형 · 물 · 신전 · 둥지 - 사용자 지시 원문 + 추가: 구조물 · 산 스케일 크게 · 설원은 맵 외곽 설산 · 매료되는 디자인 · 물리적으로 말이 되는 현실적 배치). 검증 블록 `M1-3(가)(나)`에 지형 항목을 더한다. 로드맵 = `docs/phase/roadmap-v2.md`. M1-2 결정(§4) 뒤 BR2(보스 세트 · 토벌 - 관문 자리 있음) 또는 M2(몬스터 외형 · 종). 패턴 하나만 볼 때 = `/gg boss pattern <보스> <패턴>`(끄기 `off`). 인원별 서버 부하 = `/gg perf world [crowd]`. 계절 = Workspace Attribute `Season`(spring · summer · autumn · winter).
