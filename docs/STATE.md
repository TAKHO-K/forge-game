...
> 마지막 갱신: **D1-3 결정 반영 · 2026-09-27** · 직전 보고서 = `docs/phase/D1-3-report.md`(그 전 `D1-2-report.md` · `D1-report.md` · `C1-fix5-report.md` · `A1-report.md`)(그 전 `C1-final-report.md` · `C1-report.md`) · 카툰 규칙 = `docs/art/cartoon-pipeline.md` · `docs/art/style-bible.md` v2 · 기준서 = `docs/art/ref/art-spec.md` · 에셋 교체 절차 = `docs/art/asset-pipeline.md` · 지형 굽기 = `docs/perf/streaming-settings.md` §4 · 스트리밍 = `docs/perf/streaming-settings.md` · 설계 = `docs/design/world-map-m1.md`(세계 지도 · 좌표) · 보스 = `docs/design/boss-br1-2.md` · 패턴 설명 = `docs/design/boss-patterns-explained.md`
...
| 드랍 등급 · 태초(D1) | **D1-3**: 반짝이 출현 0.1% · 골드 125마리분(골드 몫 11.1%) · 흰 발자국 삭제 · 도달 목표 = 현실 모형(`EconSimConfig.targets` - 캐주얼 1,000 = 40 ~ 45h · 상위 1% 25,300 = 2,190h ± 10% + 모형 오차 1%) · 보스 레버 L2 미적용. **D1-2**: 공격 속도 상한 ×2.5(`CombatConfig.attackSpeedMaxMultiplier`) · 치명 피해 추가분 상한 +2.2 · 딜 부위(장갑 · 신발) 태초 = 고대 ×1.25(갑옷 ×2.5 유지) · 태초 장갑 치명 +0.4 + 강공격 흰 번개 · 태초 신발 공속 · 이속 · 대시(18) 상한(`PrimordialParts`) · 반짝이 표 영웅 55 · 전설 36 · 유물 8.8 · 고대 0.195 · 태초 0.005% · 골드 25마리분 · 금화 분수 · 칭호 조건 = 드랍 태초 장비만(`TitleData.acquire`) · 환생 탭 무기 태초 문구. **드랍표**: 잡몹 유물 0.3 / 0.4 / 0.5%(tier4 ~ 6) · 고대 0.002% · 태초 0.0001%(`DropTableData` - 일반 ~ 전설 비율 유지) · 보스 첫 클리어 = 영웅 이상 보장(62 · 32.6 · 5 · 0.39 · 0.01 - 환생 0회 상향표 폐지) · 재도전 = 토벌 표(78 · 20.948 · 1 · 0.05 · 0.002) · 옛 표 = 몬스터 공정성 · 판매가 고정 기준(`fairnessGradeByTier`). **위력 가속**(드랍 장비만 `ArmorData.grades[].dropPower` 1 · 1.45 · 2.10 · 3.15 · 5.36 · 10.7 · 26.8 - 무기 환생 · 보석 · 공정성은 옛 값). **각성**(태초만 · itemLevel → 계정 최고 · `shared/Awaken`). **태초 낭만**: 세계 번호(DataStore 카운터 · Studio = 시험 키) · 전 서버 배너 · 명예의 전당(커뮤니티 광장 석판) · 흰 빛기둥 30초 · 섬광 · 흰 오라 · 이름표 ★ · 칭호 "태초의 선택"(이름표 위 무지개 · `TitleData`) · 기본 잠금 + 이중 확인 해제 · 고대 = 같은 서버 알림 · 땅 드랍 = 디아블로4식 번개(`client/DropLightning`) + 어두운 실루엣 · 일반 회색 · 출처 태그 `item.source`(모든 드랍) · `/gg drop force <등급>` | `docs/phase/D1-report.md` |
...
> **D1 끝(2026-09-27)**: 드랍표 · 위력 가속 · 각성 · 태초 낭만 적용. **EconSim 목표는 미달 - 결정 대기**(`D1-report.md` ⑨ 1 ~ 3: 모형 = 보스 장비 포함 · 상위 1% 25,300 = 1,084h · 태초 격차 추천 1안 = 딜 부위 태초 = 고대 위력). EconSim에 what-if 추가(`d1Before` · `modelBossDrops` · `primordialHold` · `primordialStep` · `dpsPrimordialStep` · `awakenCostScale` · `expLevelGap` · `expStageCap`). 로컬 EconSim 하네스 = 스크래치 `mk_econ.py` + `d1_econ_body2.luau`(병렬 프로세스 - 1프로필 약 4분).

> **D1-3 끝(2026-09-27)**: 캐주얼 1,000 39.2h(목표 하한 40의 2% 아래 - 결정) · 상위 1% 25,300 1,968h(O) · 반짝이 1인 태초 -12%(하루 알림 10만 h 39회).

> **D1-2 끝(2026-09-27)**: 상한 하나로 상위 1% 25,300 = 1,964h(목표 2,190 ± 10%의 7h 아래) · EconSim 현실 모형(보스 편차 cycle · 반짝이) · 레버 what-if · 로컬 하네스 = 스크래치 `run_cases.py` + `d12_body.luau` + `gap.py`(격차 표).

**다음** = MV1(대시 · 활강 · 낙하 · 공중 전투 · 환생 해금) → · M1-4 결정(§4 - 특히 **place 저장**) 뒤 로드맵(`docs/phase/roadmap-v2.md`): BR2(보스 세트 · 토벌 - 관문 자리 있음) · M2(몬스터 외형 · 종) · 펫 단계(알 부화 · 탑승 · 활강 - 둥지 B · C는 활강 착지 불가로 설계됨). 지형 모양을 바꾸면 `TerrainGenData.version[구역]`을 올리고 그 구역만 다시 굽고 place 저장. 패턴 하나만 볼 때 = `/gg boss pattern <보스> <패턴>`(끄기 `off`). 인원별 서버 부하 = `/gg perf world [crowd]`. 계절 = Workspace Attribute `Season`.
