...
- **플레이 검증은 이번 수정 사항만**(개정 - 사용자 지시 2026-09-26). 옛 블록은 **이번 변경이 그 블록의 코드 · 데이터를 실제로 건드렸을 때만** 돌리고, 돌린 경우 보고서에 이유 한 줄. 단계 지시(프롬프트)에 옛 블록이 적혀 있어도 이 조건이 아니면 생략하고 **생략 목록만** 보고한다(`VerifyOnly` 필터). 전체 회귀는 **마일스톤(알파 · P6)**에서만.
