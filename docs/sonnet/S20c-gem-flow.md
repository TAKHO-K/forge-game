# S20c — 보석 동선 · 홈 표현 · 장착 입력

> 의존성: S20(S20b 포함 - 보석 탭은 `panels/Inventory/GemTab.lua`, 폰 배치는 `Layout.lua`). 규모: 대(UI + 서버 확인). **S21보다 먼저 한다.**
> 이 세션은 **서버의 보석 규칙을 바꾸지 않는다.** 입력 방식(PC 더블클릭 · 우클릭 · 드래그 / 폰 탭 선택)과 홈의 표현, 분해 → 보석 탭 동선만 바꾼다. 모든 입력은 같은 함수와 같은 서버 요청을 쓴다.

## S20 결정 (PRD 20.108에 기록)

- S20 `[x]` 닫음. **B 키**는 영문에서 정상, 한글 상태에서는 두 번째 입력부터 열림(IME로 추정) - 알려진 동작으로 기록하고 튜토리얼 재설계 때 "영문 상태" 안내를 넣는다.
- 폰 / PC 경계(**폭 < 720 또는 높이 < 400 = 폰**)와 폰 시트 치수는 수용한다.
- "터치로 보석을 못 끼움"은 Fable이 아니라 **이번 세션에서 해결**한다(모바일 핵심 기능 누락).

## 0. 한 줄 목표

보석 홈을 3상태로 그리고, PC(더블클릭 · 우클릭 · 드래그)와 폰(탭 선택 → [장착] / 홈 탭)이 **같은 `GemActions.equip` / `unequip` + 같은 서버 요청**으로 보석을 끼우고 빼게 하며, 분해로 보석을 얻으면 보석 탭으로 바로 가는 동선을 만든다.

## 1. 사전 확인 (코드 전에 보고)

서버의 현재 보석 장착 · 해제 · 교체 규칙을 읽고 **코드를 고치기 전에 보고한다.** 이 규칙은 바꾸지 않는다.

- 해제 시: 보석이 가방(보석칸)으로 반환되는가 · 파괴되는가 · 비용이 드는가. 보석칸이 가득 차면 어떻게 되는가.
- 이미 찬 홈에 다른 보석을 넣을 때의 교체 방식(기존 보석은 반환 · 파괴 · 거절?). 비용이 있는가.
- 홈 등급 판정이 어디에 있는가(서버 어디 · 클라가 같은 판정을 복제하고 있는가). 어떤 홈이 어떤 등급 보석을 받는가(예: "영웅 이상 홈에만 장착 가능").
- 클라의 현재 장착 요청 경로(`GemTab.lua`의 드래그 끝 → RemoteEvent) · 서버의 거절 응답 모양(있는가).

해제 · 교체에 **비용이나 파괴가 있으면** B의 확인 overlay가 필수다. 없으면 필요 없다 - 확인 결과를 보고에 적는다.

## 2. 단계

### A. 홈 표현 (보석 탭 · 장비 상세 공통)

- **3상태**: 잠긴 홈(`HudIcons.lock`) / 열린 빈 홈(홈 등급 색 테두리 + 안쪽 어두운 음영으로 파인 느낌) / 채워진 홈(보석 아이콘 + 등급 테두리).
- 등급 색은 **기존 아이템 등급 색 표 그대로**(새 색 금지).
- 홈은 무기 그림 위치에 붙이지 말고 **무기와 무관한 한 줄 배치**로 둔다(무기 에셋은 임시 - F5에서 Socket Attachment로 교체).
- 보석 탭과 장비 상세가 **같은 홈 그리기 함수**를 쓴다(두 곳에 따로 그리지 않는다).

### B. 장착 · 해제 입력 - 모든 입력은 같은 함수(`GemActions.equip` / `unequip`)와 같은 서버 요청

- **자동 장착 규칙**: 장착 가능한 빈 홈 중 **등급이 가장 낮은 홈**. 없으면 이유 토스트 1줄(예: "장착 가능한 빈 홈이 없습니다" / "영웅 이상 홈에만 장착 가능").
- **PC 더블클릭(0.35초 이내)**: 보석칸 보석 → 자동 장착 / 끼워진 보석 → 해제. 한 번 클릭은 기존대로 상세 보기이고, **더블클릭 판정 동안 상세창이 깜빡이지 않게** 한다.
- **PC 우클릭**(마우스를 보석 위에 올린 상태, `MouseButton2`): 더블클릭과 같은 동작.
- **PC 드래그**: 드래그 시작 시 가능한 홈은 빛나고, 불가 홈은 어둡게 + 작은 ×. 불가 홈에 놓으면 **원래 칸으로 0.2초 트윈 복귀 + 이유 토스트**.
- **모바일(폰 판정)**: 더블탭 사용 안 함. **탭 = 선택 토글.** 보석칸 보석을 선택하면 [장착] 버튼(자동 장착)이 나오고 가능한 홈이 강조되며, 강조된 홈을 탭하면 그 홈에 장착한다. 끼워진 보석을 선택하면 [해제] 버튼. 다시 탭하거나 빈 곳을 탭하면 선택 해제. 버튼 터치 44 이상.
- **해제 시 보석칸이 가득 차면 해제 거절 + 토스트**(보석 소실 금지). 해제 · 교체에 비용이나 파괴가 있으면 확인 overlay 필수(사전 확인 결과에 따라).
- **판정은 서버 권위.** 클라는 미리 보여주기만 하고, 서버가 거절하면 같은 복귀 연출을 한다.

### C. 분해 → 보석 동선

- 분해로 보석을 얻으면 토스트 "보석 획득 [보석 탭 열기]"(누르면 보석 탭으로 전환). 새 보석 칸에 **NEW 표시**(한 번 보면 사라진다).
- **장비 → 분해 → 보석 탭 → 장착까지의 클릭 수를 전 · 후로 보고**한다.

## 3. 검증

- 로컬: 바뀐 · 새 파일 전부 `luau-compile` · `luau-analyze`(`2>&1`) 필수. 순수 판정(자동 장착 대상 · 가능한 홈 · 더블클릭 판정 · 해제 거절)은 순수 함수로 빼서 스텁 하네스와 자체 점검으로 재다.
- 자동 검증: **현재 세션 블록 + 건드린 모듈의 옛 블록만** 돌린다(`verify.current` 갈아 끼우기 · 동반 실행은 COMMON §3). S04(나)는 계속 제외. **검증 Play 뒤 `VerifyArmedUntil`을 바로 끈다.**
- **실제 입력 확인**: PC 마우스(더블클릭 · 우클릭 · 드래그)는 **터미널이 실제 마우스**(`user_mouse_input`)로, 모바일은 **에뮬레이터 터치**로 확인한다(가상 화면 강제 `DebugInventoryScreen` 활용). 사용자가 직접 확인할 항목은 PRD "사람이 확인할 것"에 적는다.
- 스크린샷: 홈 3상태(잠김 · 빈 홈 · 채워진 홈) · 드래그 중 가능 / 불가 홈 · 폰 [장착] / [해제] 버튼 상태 · 토스트.

## 4. 합격 기준

| # | 항목 |
|---|---|
| 1 | PC: 더블클릭 장착 · 해제, 우클릭 장착 · 해제, 드래그 성공 · 거절 복귀 |
| 2 | 자동 장착이 **가장 낮은 가능 홈**으로 간다 |
| 3 | 한 번 클릭 상세창이 깜빡이지 않는다 |
| 4 | 폰 800 × 360: 탭 선택 → [장착] · 홈 탭 장착, [해제], 거절 복귀, 선택 해제 |
| 5 | 서버 거절 시 PC · 폰이 같은 복귀 연출 · 보석칸 가득 찬 상태 해제 거절(보석 소실 0) |
| 6 | 실효 글씨 12px · 터치 44 · 상태 3종 스크린샷 · 겹침 0 |
| 7 | 서버 규칙 변경 0(사전 확인 보고와 대조) · 스크립트 에러 0 |

**진짜 합격 기준**: ① **모든 입력(더블클릭 · 우클릭 · 드래그 · 폰 버튼 · 폰 홈 탭)이 같은 함수 · 같은 서버 요청으로 가는가** ② **폰에서 터치만으로 보석을 끼우고 뺄 수 있는가.**

## 5. 하지 않는 것

- 서버의 보석 장착 · 해제 · 교체 · 등급 규칙 변경(입력 · 표현만 바꾼다).
- 새 색 · 새 에셋 · 새 파티클. 무기 그림에 홈 붙이기(F5의 일).
- 폰에서 더블탭 · 길게 누르기(탭 선택 토글만).
- 분해 · 변환권 · 리롤 규칙 변경.
