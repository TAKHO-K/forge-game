...
> 의존성: S13. 규모: 대. **S14보다 먼저 한다.** 설계 출처: 사용자 지시(2026-09-20) + PRD 20.98(S13 밸런스 결정 · 미결).
> 값은 지시가 정했다. 이 세션은 **넣고 · 재고 · 적는다.** 측정값이 기대와 다르면 값을 만지지 말고 어긋난 것만 따로 보고한다.

## 0. 한 줄 목표

딜링모드의 힐러가 힐을 쓰면 회복 대신 자신 포함 파티원에게 **쉴드 층**을 준다. 모든 피해 경로가 "쉴드 흡수 → HP" 순서를 공통 함수로 거친다.

## 1. S13 미결 결정 (PRD 20.98에 기록)

- **미결 1(힐러 버프 b 기준 이동)**: 값 바꾸지 말 것. 이번 쉴드 측정 결과와 함께 Fable에서 재측정한다.
- **미결 2 담당**: 스킬 키 라벨 · 상단 칩 · 경험치 %는 **이번 사전 작업**, 장비창은 S19 · S20, 스테이지 선택 번호는 Fable(패널 재구성과 함께).
- **미결 3**: 가방 셀 옵션 태그에도 직업색 → **S19 · S20 지시서 §6에 추가**.

## 2. 먼저 읽을 것

- PRD: 20.98(S13) · 20.81 [B-3](파티 회복) · 20.64(힐러 버프) · 20.67 [2] · [8](딜링모드) · 20.44 [1](측정표)
- 코드: `server/HealCast.lua` · `server/HealerDealingMode.server.lua` · `server/SkillServer.server.lua`의 `castHeal` · `server/PlayerDamage.lua` · `server/PlayerState.lua`(`getHp` · `setHp`) · `server/BuffState.lua` · `server/PartyState.lua` ·
  `client/PartyHealView.client.lua` · `client/PlayerHealthBar.client.lua` · `client/PartyHud.client.lua`(+ `client/hud/`) · `shared/BalanceSim.lua` · `shared/data/SkillData.lua`(healer)

## 3. 사전 작업 (커밋 후 본 작업)

- 스킬 키 라벨 3 · 상단 칩 2 · 경험치 % 1곳을 **실효 12px 이상**으로(`Theme.effectiveTextSize` 기준 - COMMON.md §2). 넘치면 칸을 키우되 `ScreenMap` 겹침 0을 유지한다(기존 4건 제외).
- **PC 스크린샷 전후.**

## 4. 본 작업: 힐러 딜링모드 쉴드

1. **동작**: 딜링모드에서 힐을 쓰면 회복 대신 **자신 포함 파티원에게 쉴드**. 힐에 붙어 있던 버프는 쉴드를 줄 때도 그대로 적용. **치명 판정은 힐과 같은 함수를 쓴다**(힐 치명 규칙이 나중에 바뀌면 쉴드도 같이 바뀌게).
2. **구조**: 대상마다 쉴드 층 목록 `{양, 만료 시각, 시전자}`. 피해는 **만료가 가장 빠른 층부터** 깎고, 0이 된 층은 즉시 제거. 만료된 층은 남은 양과 함께 사라진다(먼저 쓴 쉴드부터 순서대로 사라짐, 다 깎였으면 사라질 것 없음).
3. **겹침 규칙**:
   - 대상 1명당 **최대 4겹**. 5겹째는 걸리지 않는다.
   - 대상에게 이미 쉴드가 **2겹 이상** 있으면 새 쉴드량은 **50%**(3 · 4번째 층은 반감). 판정은 **시전 순간의 겹 수** 기준.
   - **같은 시전자가 같은 대상에게 다시 쓰면** 새 층을 추가하지 않고 자기 기존 층을 새 쉴드로 **교체**한다(시전자당 대상별 1겹). 교체 시 반감 판정은 **자기 층을 뺀 겹 수**로 한다.
   - 쉴드량 = 힐량 × 0.6 × 치명 배율 × (반감이면 0.5). **모두 설정값**(`shared/data/`).
4. **가동률**: 힐러 1명당 지속시간 · 쿨타임으로 조절. 설정값으로 빼고 초안은 **1인 기준 대상별 쉴드 가동률 약 40 ~ 50%**.
5. **서버 권위**: 모든 피해 경로가 "쉴드 흡수 → HP" 순서를 거치게 **공통 함수**로. `Humanoid:TakeDamage` · `Health` 직접 대입 등 **우회 경로를 전부 찾아 목록으로 보고**하고 공통 함수로 바꾼다. 피해 이벤트에 **`ignoresShield` 플래그 자리**를 만든다(지금은 낙사 등 판정형만, 보스 즉사 패턴은 Fable에서 지정).
6. **UI**: 내 체력바와 파티 HUD 체력바 위에 **흰 쉴드 막대(총량)**. 새 HUD 요소 없이 기존 바 위에 겹쳐 그린다. 스크린샷 1회로 실제 보이는지 확인.
7. **저장 없음**(`SAVE_VERSION` 변경 없음).

## 5. 측정 (보고만, 값 확정 금지)

- 기존 DPS · 처치 시간 측정 도구로 **딜러4 / 딜러3+힐러1 / 딜러2+힐러2 / 딜러1+힐러3 / 힐러4**의 보스 처치 시간 · 받은 피해 · 쉴드 흡수량 · 대상별 쉴드 가동률 · 평균 겹 수를 표로.
- 기준:
  - 딜러3+힐러1 ≈ 딜러4(기존 앵커)
  - 딜러2+힐러2는 약간 느리지만 받은 피해가 크게 적음
  - 힐러3 ~ 4는 가동률은 높지만 반감 때문에 흡수량 증가가 둔하고 처치 시간은 확실히 느림
  - **어긋난 값만 따로 보고한다.**
- ★① **쌍검 ÷ 대검 1.3201이 변하지 않았는지** 확인.

## 6. 자동 검증

**(가)** 순수 함수 · 합성 데이터

- 층 순서(만료 빠른 층부터 흡수) · 다 깎인 층 제거 · 만료 제거 · 3 · 4번째 층 반감 · 5겹째 거절 · 같은 시전자 재시전 시 교체(겹 수 불변) · 버프 유지 · 치명 시 쉴드량 증폭 · `ignoresShield` 피해는 쉴드 통과 · **우회 피해 경로 0**.

**(나)** 실제 Player

- 2인 이상 파티에서 쉴드 표시 · 흡수 실제 동작, 스크린샷.

현재 세션 블록 + **건드린 모듈의 옛 블록만**(피해 공통 함수를 건드리므로 **보스 패턴 옛 블록 동반 실행 포함**). S04(나)는 계속 제외.

## 7. 멈춤 조건

- 피해 경로를 공통 함수로 바꾸다가 **기존 보스 검증 블록 결과가 달라지면 멈추고 보고**한다.

## 8. 하지 않는 것

- 미결 1(힐러 버프 b) 값 변경 · 보스 즉사 패턴의 `ignoresShield` 지정(Fable) · 장비창 · 스테이지 선택 번호 글씨(S19 · S20 · Fable) · 부활(F4).
