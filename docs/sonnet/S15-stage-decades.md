# S15 — 스테이지 선택 10단위 UI + station 등록

> 의존성: S11(같은 파일 - 보상 띠가 먼저 들어가 있어야 한다) · S06. 규모: 중. 설계 출처: **PRD 20.81 [C-4]** · [D-1](station) · 20.71 [3].

## 0. 한 줄 목표

스테이지 그리드를 **10칸(5 × 2) = 한 구간(1 ~ 10 · 11 ~ 20 …)**으로 바꾸고 그 위에 구간 칩 5개를 둔다. 패널을 `UIManager`의 `station`으로 등록한다. 이동 · 거절 · 투표 · 보상 띠의 동작은 한 글자도 안 바뀐다.

## 1. 먼저 읽을 것

- PRD: **20.81 [C-4] · [D-1]** · 20.71 [3](패널 규칙 · 임의 결정 9 "클라가 판정하지 않는다") · S11이 남긴 절
- 코드: `client/StageSelectPanel.lua` 전체(`COLUMNS` · `ROWS` · `WINDOW_SIZE` · `windowStart` · 칸 상태 O/△/X 계산 · 페이지 버튼 · 여는 곳) · `client/StageRewardBand.lua`(S11) · `client/StageUI.client.lua`(패널을 여는 버튼) · `client/UIManager.lua`(S06의 `kind`) · `client/ui/PanelRegistry.lua`

## 2. 단계

1. **그리드**: `COLUMNS = 5` · `ROWS = 2` → 10칸. 창의 시작 = `floor((기준 스테이지 − 1) ÷ 10) × 10 + 1`(**항상 구간에 정렬** - 보스 칸이 언제나 5번째 · 10번째 자리). 칸 크기는 남는 폭에 맞춰 키운다(모바일 44 이상). 패널 높이는 S11의 **396 그대로** - 3행 → 2행으로 줄어든 높이에 구간 띠(높이 32)가 들어간다. 고정 크기만.
2. **구간 띠**(그리드 위): 칩 5개 `41-50` `51-60` … + 양끝 ◀ ▶(칩 묶음을 50스테이지씩 이동). 띠의 시작도 50에 정렬. 칩 상태:
   - 그 구간의 끝 ≤ 최고 클리어 → ✓ + `textTertiary` / 최전선(`best + 1`)을 포함 → ● + `ember` / 아직 시작 못 함(구간의 첫 스테이지 > `best + 1`) → `lockedIcon` 색. **잠긴 칩도 눌린다**(다음 목표를 보는 것 - 이동 가능 여부는 서버가 판정).
   - 지금 보고 있는 구간의 칩 = `rimHi` 테두리.
3. **버튼 줄**: [◀ 이전 10] [최전선] [다음 10 ▶]은 남긴다(구간 하나씩 이동).
4. **station 등록**: `UIManager.register("stageSelect", { kind = "station", hotkey = M(PanelRegistry), hasCloseButton = true, … })`. 패널이 스스로 하던 열기/닫기 · DisplayOrder(10)는 `UIManager`에 넘긴다. 가방(window)을 열면 닫히고, 강화 패널(station)과 서로 닫는다. **견습 중에는 안 열린다**(기존 규칙 - 단축키 M에도 같은 검사).
5. 스테이지 번호가 매우 클 때(예: 12,345)의 칩 글씨가 칸을 넘지 않는지 - 넘으면 `1234x` 식 축약이 아니라 글씨를 `caption`(12)로 내리고 칩 폭을 고정 분할로 둔다.

## 3. 검증

- **로컬**: `luau-compile` · 최상위 local ≤ 120.
- **Play 1(자동)**: 27-4 (가)(나) 11/11 · S11 검증 회귀 없음 · `[S06][UI] 겹침 0쌍` · 에러 0.
- **Play 2(클릭 · 스크린샷)**: `/gg`로 최고 클리어를 47로 두고 ① 열면 41 ~ 50 구간 · 칩 `41-50`에 ● ② 보스 칸 45 · 50이 5 · 10번째 자리 ③ ◀ ▶로 칩 묶음 이동(1-50 ↔ 51-100) ④ 잠긴 구간 칩을 눌러 61 ~ 70을 보고 보스 칸 → 띠 → [도전] → 서버 거절 사유가 상태줄에 ⑤ 일반 칸 클릭 = 즉시 이동(기존) ⑥ M 키 토글 · X/Backspace 닫기 · 가방을 열면 닫힘 ⑦ 최고 클리어 12,340에서 칩 글씨 ⑧ 폰 에뮬레이터.

## 4. 합격 기준

| # | 항목 |
|---|---|
| 1 | 스크린샷 ① ~ ⑧ · 자동 검증 회귀 없음 · 에러 0 |
| 2 | 이동 요청 · 거절 · 투표 · 보상 띠의 코드 경로 변경 0(`git diff`에서 서버 파일 변경 0) |

**진짜 합격 기준**: ① 어느 구간을 봐도 **보스 칸이 5 · 10번째 자리**에 있다. ② 서버 파일 변경 0 - 클라 표시만 바꿨다.

## 5. 하지 않는 것

- 하위층 일괄 클리어 · 일괄 보상 · 보스 건너뛰기(F1). 100단위 상위 화면. 저장 구조 변경.
