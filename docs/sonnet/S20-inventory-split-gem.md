# S20 — `InventoryUI` 분할 ②: 보석 탭 (동작 변경 0)

> 의존성: S19(같은 절차를 한 번 해 본 뒤). 규모: 중. 설계 출처: **PRD 20.81 [D-4] · [D-6] 8번**.
> S19와 같은 규칙이다 - **옮기기만 한다.** 보석 탭은 드래그 장착 · 등급 상한 거부 · 분해 · 변환권 리롤이 얽혀 있어 파티 탭보다 공용 상태를 많이 읽는다.

## 0. 한 줄 목표

장비창의 **보석 탭**(23-2 ~ 23-4 · 26-3: 보석 가방 · 무기 홈 5칸 · 드래그 장착 · 등급 상한 거부 · 옵션 표시 · 변환권 구매/리롤)을 `client/panels/Inventory/GemTab.lua`로 떼어 낸다.

## 1. 먼저 읽을 것

- PRD: 20.57 · 20.58 [4](클리핑 버그) · 20.59(드래그 장착 · 등급 상한 · 창 위치) · 20.67 [10] · [12](옵션 게이지 UI) · S19가 남긴 절(대조표의 모양 · `deps` 경계의 선례)
- 코드: `client/InventoryUI.client.lua` **전체**(S19 뒤의 모습) - 전방 선언 `updateGemTab` · `cancelGemDrag` · 선택 상태(`kind = "gemSlot"` · `"gemBag"`) · 드래그 중 입력 처리(`UserInputService` 연결) · 창을 닫을 때 드래그 취소(`onClose`) · 상세(Detail) 패널이 보석을 그리는 경로

## 2. 단계

1. **목록 작성**(S19의 ① ~ ⑥ 그대로) + ⑦ **선택 상태 · 상세 패널과의 접점**: 보석 탭이 공용 선택 상태를 어떻게 쓰고, 상세 패널이 보석 선택을 어떻게 그리는지. 이 접점은 **옮기지 않는다** - 상세 패널은 `InventoryUI`에 남고, 보석 탭은 `deps.select(kind, value)` · `deps.getSelection()`으로만 만난다.
2. **경계**: `GemTab.create(deps) -> { frame, update, cancelDrag, onOpen, onClose }`. 드래그 중의 전역 입력 연결은 새 모듈 안으로 같이 옮기되, **연결을 끊는 시점**(`onClose` · 탭 전환 · 리스폰)이 전과 같은지 목록 ⑥에 적고 대조한다.
3. **이동**: 잘라서 붙인다. 다시 쓰지 않는다.
4. **레지스터 검사**: 두 파일 `luau-compile` 통과 · 최상위 local 수 전/후 기록. `InventoryUI`가 아직 120을 넘으면 **넘는다는 사실만 기록**한다(가방 · 장비 분할은 다음 세션의 일 - 이 세션에서 더 쪼개지 않는다).
5. 모바일의 "탭 → 탭 장착" 경로(20.81 [D-5])는 **만들지 않는다**(새 기능이다 - 분할이 끝난 뒤).

## 3. 검증

- **대조**: 목록 ① ~ ⑦ 빠진 것 0.
- **Play 1(자동)**: 26-2 41/41 · 26-3 6/6 · 23-x 관련 검증 · S01(변환권 가격) 회귀 없음 · 에러 0.
- **Play 2(실클릭 · 스크린샷 - "전"을 먼저 찍는다)**: 보석 탭 열기 · 보석을 홈으로 **드래그 장착** · 홈에서 빼기 · 등급 상한에 걸리는 보석 → 거부 문구 · 드래그 도중 X로 창 닫기(드래그가 취소되고 유령 아이콘이 안 남는다) · 드래그 도중 탭 전환 · 분해 · 변환권 구매 → 리롤 → 옵션 게이지 갱신 · 상세 패널의 보석 표시 · 창 위치 저장(옮긴 뒤 다시 열기).

## 4. 합격 기준

| # | 항목 |
|---|---|
| 1 | 대조표 빠진 것 0 · 로직이 바뀐 줄 0(`--color-moved`) |
| 2 | 실클릭 전 항목이 전과 같다 - 특히 **드래그 도중 닫기/탭 전환에서 유령 상태 0** |
| 3 | 자동 검증 회귀 없음 · 에러 0 · 서버 파일 변경 0 |

**진짜 합격 기준**: ① **드래그 취소 경로 3개(닫기 · 탭 전환 · 리스폰)**가 전부 전과 같이 돈다. ② 대조표에 구멍이 없다.

## 5. 하지 않는 것

- 모양 · 문구 · 기능 변경 · 버그 수정(보고만) · 가방/장비 탭 분할 · kit 부품 교체.
