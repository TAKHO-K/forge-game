# S10 — 파티원 드랍 알림 (`DropFeed`)

> 의존성: S01(드랍이 배열 · 보스는 가방 직행) · S06(`Toast`의 TR 줄). 규모: 중. 설계 출처: PRD 20.73 [5-3] 표 전체.

## 0. 한 줄 목표

유물 · 고대 · 태초 장비가 **굴려진 순간** 같은 파티 전원(본인 포함)의 화면 오른쪽 위에 "OOO: 유물 장갑 Lv.52"가 뜬다. 태초는 같은 서버 전원 + 채팅 1줄.

## 1. 먼저 읽을 것

- PRD: **20.73 [5-3]**(표의 모든 행이 명세다) · 20.81 [D-2](TR 구역) · [D-3](`Toast`)
- 코드: `server/CombatResolution.lua`의 `grantKillReward`(S01 뒤의 모습 - 드랍이 굴려지는 자리 · 보스는 가방 직행) · `server/PartyState.lua`(`getParty` · `getMemberPlayers`) · `shared/data/ItemVisualData.lua`(등급색 · 등급 표시명) · `shared/data/EquipSlots.lua`(부위 표시명) ·
  `client/ui/kit/Toast.lua`(`TR` 줄 · `groupKey` · `priority`) · `client/ui/ScreenMap.lua`(`TR.dropFeed`) · 태초 rainbow 테두리 코드(grep `rainbow`) · 채팅 시스템 메시지 경로(S08이 만들었으면 그것)

## 2. 단계

1. **데이터** `shared/data/DropNoticeData.lua`: `partyGrades = { relic, ancient, primal }`(실제 등급 id는 `ArmorData.gradeOrder`에서 확인) · `serverWideGrades = { primal }` · `seconds = { default = 5, primal = 8 }` · `maxRows = 3` · `queueMax = 8` · `groupWindowSeconds = 1`.
2. **서버**: `grantKillReward`가 아이템을 굴린 직후(땅 스폰 · 가방 직행 **둘 다**의 앞) `DropNotice.publish(recipient, item)`(새 모듈 `server/DropNotice.lua`):
   - 등급이 `partyGrades`에 없으면 끝. 받는 사람이 파티면 → 같은 서버의 파티 멤버 전원(본인 포함)에게, **솔로면 유물 · 고대는 보내지 않는다**.
   - 등급이 `serverWideGrades`면 → 서버 전원에게 + 채팅 시스템 메시지 1줄.
   - payload = `{ name, grade, part, itemLevel }`만(옵션 · 능력치를 보내지 않는다).
   - **제외**: 견습의 확정 지급 · 대여(`TutorialState` 경로) · 분해로 나온 보석 · 상점 구매. `publish`를 `grantKillReward`에서만 부르면 자동으로 제외된다 - 다른 곳에서 부르지 않는다.
   - `MessagingService`로 다른 서버에 뿌리지 않는다.
3. **클라** `client/hud/DropFeed.client.lua`(신규 - 기존 HUD 파일에 넣지 않는다): RemoteEvent `DropNotice` → `Toast.push("TR", { richParts, seconds, groupKey = name, priority })`.
   - 문구: `"OOO: "`(`textPrimary`) + `"유물 장갑"`(**등급색** - `ItemVisualData`) + `" Lv.52"`(`textPrimary`). 태초: `"★ OOO님이 태초 갑옷을 얻었습니다"` + 무지개 테두리.
   - 묶기: 같은 사람 1초 안 여러 건 → "OOO: 유물 장갑 외 1". 넘침: 대기열 8, 넘치면 **유물부터** 버린다(`priority`: 유물 1 · 고대 2 · 태초 3).
   - 끄기 설정은 만들지 않는다.
4. **DevTools**: `/gg dropnotice <grade> [n]` - 가짜 알림 n건을 자기 파티(또는 자신)에게 주입(검증 · 스크린샷용).

## 3. 자동 검증

**(가)**: 등급 필터(영웅 · 전설 = 발신 0) · 묶기 · 대기열 넘침의 버림 순서는 `Toast`의 순수 부분을 서버에서 못 돌리므로 **클라 자체 점검**으로: Studio에서만 도는 `DropFeed`의 `selfTest()`가 접속 12초 뒤 합성 이벤트 12건(유물 9 · 고대 2 · 태초 1)을 넣고 `[S10][UI] 표시 3행 · 대기 8 · 버린 것 = 유물 1` 식으로 클라 콘솔에 찍는다.
**(나)** 실제 Player + 더미 파티: ① 유물을 강제로 굴린다(검증 안에서 `Loot`의 결과를 직접 만들어 `grantKillReward`가 쓰는 `publish`를 부른다) → 수신 대상 = 실제 Player 1(더미에게 `FireClient`를 시도하지 않는다 - **에러 0**이 합격) ② 솔로 · 유물 → 발신 0 ③ 솔로 · 태초 → 서버 전원 발신 + 채팅 1줄 ④ 견습 확정 지급 경로 → 발신 0 ⑤ 보스 첫 클리어(가방 직행)에서도 발신된다.

## 4. 합격 기준

| # | 항목 |
|---|---|
| 1 | (나) 5/5 · 클라 자체 점검 O · 회귀 없음 · 에러 0 |
| 2 | 스크린샷: `/gg dropnotice`로 ① 1건 ② 4건 동시(3행 + 5초 뒤 네 번째) ③ 태초(무지개 테두리 8초 + 채팅 줄) ④ 같은 사람 2건 묶음 ⑤ 폰 에뮬레이터 |
| 3 | `[S06][UI] 겹침 0쌍`(채팅 · 파티 목록 · 토스트 줄 · 투표 패널과 안 겹친다) |

**진짜 합격 기준**: ① **더미/스탠드인이 낀 파티에서 서버 에러 0**(Player가 아닌 멤버에게 쏘지 않는다). ② 4건 동시 주입 스크린샷에서 3행이 넘지 않고 네 번째가 뒤에 나온다.

## 5. 하지 않는 것

- 줍는 순간의 알림(기존 획득 팝업 그대로) · 설정 항목 · 다른 서버 전파 · 같은 사람 쿨다운(두지 않는 것이 결정).
