# S21 — 무료 모델 안 스크립트(백도어) 전수 검사 — 읽기 전용

> 의존성: 없음 - **에셋 교체(힉스필드) 전이면 언제든.** 규모: 소. 설계 출처: **PRD 20.81 [E] 7번**.
> 이 세션은 **아무것도 지우거나 고치지 않는다.** 찾아서 표로 보고한다. 지우는 결정은 사용자 몫이다.

## 0. 한 줄 목표

Studio의 place 안에서 **Rojo가 관리하지 않는 모든 스크립트**(무료 모델에 딸려 들어온 것)를 전부 찾아내고, 백도어 · 원격 코드 실행 · 외부 통신 · 결제/텔레포트 유도 패턴이 있는지 검사해 표로 남긴다.

## 1. 먼저 읽을 것

- PRD: 20.81 [E] · 20.49 · 20.51 · 20.55(지형 · 무료 모델이 들어온 이력) · 20.53(임포트 경로)
- `roblox/default.project.json` - Rojo가 관리하는 곳은 `ServerScriptService` · `ReplicatedStorage.Shared` · `StarterPlayer.StarterPlayerScripts` 셋이다. **그 밖의 모든 스크립트가 검사 대상이다.**

## 2. 단계 (Edit 모드 - Play를 하지 않는다. 이 세션은 공통 규칙의 "Play 절차"를 쓰지 않는다)

1. **전수 나열**(MCP의 데이터 모델 탐색 · `execute_luau` - `shared` 모듈을 `require`하지 않는 순수 탐색 코드만): `game:GetDescendants()` 중 `LuaSourceContainer`(`Script` · `LocalScript` · `ModuleScript`) 전부를 서비스별로. 출력 = 전체 경로 · 클래스 · `Enabled`/`Disabled` · `RunContext` · 소스 길이.
   - 돌 곳: `Workspace` · `ServerStorage` · `ReplicatedStorage`(Shared 밖) · `ReplicatedFirst` · `StarterGui` · `StarterPack` · `StarterPlayer`(StarterCharacterScripts 포함) · `Lighting` · `SoundService` · `Teams` · `Chat` · `TextChatService` · `MaterialService` · 그 밖 `GetChildren`으로 보이는 서비스 전부.
   - Rojo 관리 구역 안에서도 **`roblox/src`에 대응 파일이 없는 스크립트**가 있으면 나열한다(Studio에서 누가 직접 넣은 것).
2. **소스 검사**(MCP `script_read` · `script_grep`): 각 스크립트에서 아래 패턴을 찾는다.

   | 분류 | 패턴 |
   |---|---|
   | 원격 코드 | `require(` 뒤에 숫자(에셋 id) · `require(` 뒤에 변수로 조립된 숫자 · `loadstring` · `getfenv` · `setfenv` · `InsertService` · `LoadAsset` |
   | 외부 통신 | `HttpService` · `GetAsync` · `PostAsync` · `RequestAsync` · URL 문자열 · `MessagingService` |
   | 유도 | `MarketplaceService`(`PromptPurchase` · `PromptGamePassPurchase` · `PromptProductPurchase`) · `TeleportService` · `PromptGameInvite` |
   | 권한 | `:Kick(` · `Players.PlayerAdded` 안의 UserId 비교(관리자 백도어) · `game.CreatorId` 비교 · 채팅 명령 파싱(`Chatted`) |
   | 난독화 | 한 줄 1,000자 이상 · `\x`/`\u` 이스케이프 덩어리 · `string.char(` 연쇄 · `string.reverse` · base64처럼 보이는 긴 문자열 · 의미 없는 변수명 연쇄(`l1lI1`) |
   | 숨김 | 이름이 공백 · 제어 문자 · 흔한 위장명(`Fix` · `Weld` · `Anti Lag` · `Vaccine` · `Spread` · `Infection`) · `Parent`를 실행 중에 옮기는 코드 · `script:Destroy()`로 자기를 지우는 코드 · `Archivable = false` |
   | 그 밖 | `RemoteEvent` · `RemoteFunction`을 스스로 만드는 코드(`Instance.new("RemoteEvent")`) - 클라가 부를 수 있는 서버 입구 |

3. **비스크립트 위험물**도 나열: Rojo 밖의 `RemoteEvent` · `RemoteFunction` · `BindableEvent` 인스턴스 · `ModuleScript`를 담은 `Model`/`Folder`의 출처를 알 수 없는 것.
4. **판정**: 스크립트마다 `깨끗함`(패턴 0 · 소스를 읽어 용도가 분명) / `확인 필요`(패턴은 있으나 용도가 설명된다 - 설명을 적는다) / `위험`(원격 코드 · 난독화 · 숨김 중 하나라도). **판정이 애매하면 `확인 필요`로 두고 이유를 적는다 - 임의로 `깨끗함`에 넣지 않는다.**
5. **그 모델이 필요한가**: 스크립트를 품은 무료 모델마다 "스크립트를 지워도 모델이 멀쩡한가"(장식 모델의 용접/애니메이션 스크립트는 대개 불필요)를 **추정으로만** 적는다. 시험 삼아 지우지 않는다.
6. 결과를 `docs/asset-script-audit.md`(신규)와 PRD 기록에: 서비스별 개수 요약 · 전체 표(경로 · 클래스 · 길이 · 걸린 패턴 · 판정 · 메모) · 위험/확인 필요 항목의 소스 발췌(10줄 이내) · **사용자가 결정할 목록**("이 n개를 지울지").

## 3. 검증

- 나열의 완전성: `LuaSourceContainer` 총개수 = Rojo 관리 개수(= `roblox/src`의 파일 수와 대조) + 검사 대상 개수. 두 수의 합이 맞는지 로그로.
- 검사 대상이 0개라면 그것이 결과다 - "0개"를 위 합계 검산과 함께 보고한다.
- `git diff --stat`: 코드 파일 변경 0 · 새 문서 1개 + PRD.
- Studio의 place를 **저장하지 않는다**(탐색만 했다 - 변경이 없어야 한다).
...
|---|---|
| 1 | 합계 검산이 맞다(전수라는 증명) |
| 2 | 검사 대상 스크립트 전부에 판정과 근거가 있다 |
| 3 | 인스턴스 · 코드 변경 0 |

**진짜 합격 기준**: ① **합계 검산** - 빠뜨린 서비스가 없다. ② `위험` · `확인 필요` 항목마다 사용자가 읽고 결정할 수 있는 소스 발췌가 붙어 있다.

## 5. 하지 않는 것

- 삭제 · 비활성화 · 수정. 에셋 교체. Play 실행. 판정이 애매한 것을 깨끗하다고 쓰는 것.
