...
> **⚠ 전제 확인(2026-09-20 S13 시작 때 발견)**: 파티 탭은 S12b에서 이미 독립 창(`panels/Party.lua` · HUD [파티] 버튼 · P 키)으로 분리됐고 장비창에는 `장비` · `보석` 탭만 남았다(`InventoryUI` 145 · 526줄 주석). 그래서 이 세션의 "파티 탭을 `PartyTab.lua`로 옮긴다"는 대상이 바뀌었을 수 있다. **1단계 목록을 짤 때 파티 탭에 속한 함수 · 리스너가 `InventoryUI`에 남아 있는지 먼저 확인한다.** 하나도 안 남았다면 옮길 것이 없다 - 그 사실과 목록을 PRD에 적고 §0으로 멈춘다(S19를 건너뛰고 S20으로 갈지 Fable이 정한다). 남은 것이 있으면 계속한다. 아래 §6은 그와 무관하게 적용된다.
...
## 6. S12b 결정으로 추가된 요구 (2026-09-20 · PRD 20.97 [8] 미결 2 · 6 - 사용자 결정)

S19 · S20 공통이다. 이 절의 네 항목은 위 "동작 · 모양 · 문구 변경 0" 규칙의 **유일한 예외**이고, 이동 커밋과 **같은 커밋에 섞지 않는다**(별도 커밋 - 어느 쪽이 깨뜨렸는지 가려야 한다).

1. **장비창을 폰용 스크롤 창으로 재구성한다**(미결 2). 지금 장비창은 720 × 560 고정 캔버스를 `UIScale`로 통째로 줄여(`InventoryUI`의 `winScale` · `contentScale`) PC 1321 × 484에서 배율 0.769 → 명목 12가 **9.2px**로 보인다. 분할(이동)을 끝낸 뒤 **S20의 마지막 단계**에서 Panel kit의 스크롤 창 구조로 다시 짓는다: 제목줄 · 하단 고정 줄은 스크롤 밖, 본문만 `ScrollingFrame`(COMMON.md §2 "패널은 화면보다 클 수 없다"). 고정 캔버스를 `UIScale`로 줄이는 구조는 남기지 않는다.
2. **합격 기준(추가): UIScale을 반영한 실효 글씨 ≥ 12px.** 확인은 자체 점검으로 한다 - `[S12b][UI] 장비창 글씨(장비 탭)`가 **X → O**, `[S12][UI][글씨] inventory(window)`의 실효 12 미만이 **0개**. PC(1321 × 484)와 폰 배율(842 × 534, `ForceTouchLayout`) 둘 다. (S13 사전 작업 실측 기준선: 실효 최소 9.2px · 12 미만 27개 - 이 값이 S19 시작 때의 "전"이다. S19는 이 줄을 PRD 기록에 붙인다.)
3. **장비 문구는 `ItemDescribe` 하나로 통합한다**(미결 6). `InventoryUI`의 상세 패널 · 툴팁이 자기 코드로 만드는 옵션 · 등급 · 강화 · 세트 줄을 `shared/ItemDescribe.lua` 호출로 바꾸고, 같은 문구를 만드는 코드를 두 곳에 남기지 않는다(이중 유지 0). 바꾼 뒤 문구가 이전과 같은지는 대조표(항목 · 이전 문구 · 이후 문구)로 확인한다.
4. **가방 셀의 옵션 태그에도 직업색을 입힌다**(S13 미결 3 · PRD 20.98 [9] - 사용자 결정 2026-09-20, S13b 지시). S13은 장비 · 보석 툴팁과 상세 패널의 옵션 줄에만 직업 특화 옵션 색(`ItemDescribe.optionLines`의 `accentClassId` → `UIColors.classAccent`)을 입혔다. 가방 셀의 옵션 태그(`InventoryUI`의 `optionTag` - 지금 `mismatched and textTertiary or optionVisual.color or textPrimary`)에도 같은 규칙을 적용한다: 직업 특화 옵션 = 직업색 · **직업 불일치는 회색이 우선** · 공통 옵션은 그대로. 새 색 금지 - 규칙을 셀 코드에 복제하지 말고 `ItemDescribe`가 주는 `accentClassId`를 그대로 쓴다(3항의 통합과 같은 방향).
