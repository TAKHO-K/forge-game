# S12 — A안: 견습 중 파티 허용 · 친구 부르기 · 문구 · 로딩

> 의존성: S09(문구가 "경험치 최대 +20%"를 말한다). 규모: 중. 설계 출처: PRD 20.73 [7-1] · [7-2].

## 0. 한 줄 목표

"레벨이 달라도 바로 같이 사냥"을 게임의 얼굴로 세운다: 견습 중에도 **파티 결성은 된다**(견습 보스는 계속 싱글 · 파티 보스는 견습 중인 멤버를 빼고 간다), 친구 부르기 버튼과 로딩 · 안내 문구를 넣는다.

## 1. 먼저 읽을 것

- PRD: **20.73 [7-1] 표 · [7-2] 표 전체** · 20.62 [6] · 20.70(투표) · 20.56(견습)
- 코드: `server/PartyServer.server.lua`(`checkJoinable` - `tutorial_self` · `tutorial_target`) · `server/TutorialState.lua`(`start` - 시작 시 파티 자동 탈퇴 · `getMonsterStage` · 견습 중인지 묻는 함수) · `server/BossEncounter.lua`(`spawnForParty` · `checkPartyEntry` · 입장 머릿수 N) ·
  투표 모듈(grep `PartyVote`) · `/gg party selftest`(DevTools) · `client/TutorialHud.client.lua` · `client/ClassSelectUI.client.lua` · `client/InventoryUI.client.lua`의 파티 탭 버튼 줄 · `ReplicatedFirst` 매핑이 `default.project.json`에 **없다**는 것

## 2. 단계

### 단계 1 — 견습 중 파티 허용 (20.73 [7-2] "바꿀 곳" 표 그대로)

| 곳 | 바꾸는 것 |
|---|---|
| `PartyServer.checkJoinable` | `tutorial_self` · `tutorial_target` 거절 제거 |
| `TutorialState.start` | 시작 시 파티 자동 탈퇴 제거 |
| `BossEncounter.spawnForParty` · `checkPartyEntry` · 투표 | **대상 = 견습 중이 아닌 멤버만.** 입장 머릿수 N(보스 HP 배수)도 그 수. 견습 중인 멤버는 사냥터에 남고 "파티가 보스전에 들어갔습니다" 알림(기존 파티 토스트)만 받는다. 투표 동의자도 견습 중이 아닌 멤버에서 센다 - 대상이 리더뿐이면 솔로처럼 즉시 입장 |
| 리더가 견습 중 | 리더는 파티 보스를 열 수 없다(기존 견습 제한 그대로 - 거절 사유 문구는 기존 것) |
| 견습 보스 | 그대로 싱글(`spawnTutorialFor`). 파티원은 따라 들어가지 않는다 |
| 보상 | 코드 변경 없음 - 견습 중인 사람은 견습 스테이지 기준, 친구는 자기 스테이지 기준(`getMonsterStage`가 이미 그렇게 준다). 파티 경험치 보너스는 양쪽 모두 |
| 입장 밴드(rec + 1) | **넓히지 않는다** |

### 단계 2 — 친구 부르기

- 버튼 [친구 부르기] = `SocialService:PromptGameInvite(player)`(로블록스 기본 초대 창). `SocialService:CanSendGameInviteAsync`가 false거나 실패하면 버튼을 숨긴다(`pcall`).
- 자리: ① `TutorialHud` 진행 칩 옆 - **견습 1단계에서만** 강조(`ember` 테두리), 2단계부터는 숨김 ② 장비창 파티 탭의 버튼 줄에 상시.
- 친구가 같은 서버에 들어온 순간: 서버가 `Player:IsFriendsWith`(`pcall` · 접속 시 1회 · 기존 멤버 각각에 대해)로 확인 → 그 친구에게 토스트 1회 "친구 OOO님이 들어왔습니다 [파티 초대]" - 버튼은 기존 초대 요청을 그대로 쏜다. S06이 끝났으면 `Toast`의 TC 줄, 아니면 기존 `PartyHud` 토스트.

### 단계 3 — 문구 (20.73 [7-1]의 초안을 **그대로** 쓴다 - 새 문장을 짓지 않는다)

- 직업 선택 패널 맨 위 한 줄 · 견습 1단계 안내 뒤 한 줄.
- 로딩: `src/first/` 폴더를 새로 만들고 `default.project.json`에 `ReplicatedFirst` 매핑을 추가한다 → `LoadingTip.client.lua`: 문구 ① ~ ④ 중 무작위 1개를 화면 하단에 띄우고 `game:IsLoaded()` 뒤 1초에 페이드아웃. 이미지 없음 · 로블록스 기본 로딩 화면은 **끄지 않는다**(`RemoveDefaultLoadingScreen`을 부르지 않는다). 문구 ④(강화 +19)는 S03이 끝났을 때만 넣는다.
- 문구 표는 `shared/data/`가 아니라 `src/first/LoadingTips.lua`(ReplicatedFirst는 shared보다 먼저 뜬다).
- 게임 페이지 설명 · 썸네일 · 제목은 사용자 몫 - "사람이 할 것"에 초안과 함께 남긴다.

## 3. 자동 검증

`/gg party selftest`(스탠드인)에 항목 추가 + (나):

| # | 검사 | 기대 |
|---|---|---|
| 1 | 견습 중인 스탠드인을 초대 · 견습 중인 쪽이 초대 | 둘 다 성립(거절 사유 없음) |
| 2 | 견습 시작 | 파티 유지 |
| 3 | 4인 파티 중 1명이 견습 중 → 파티 보스 입장 | encounter 멤버 3 · HP 배수 = 3인분(`N^p`, N = 3) · 견습 멤버는 사냥터에 그대로 |
| 4 | 3번에서 투표 | 동의자는 견습 아닌 멤버에서만 센다 |
| 5 | 리더 빼고 전원 견습 | 솔로처럼 즉시 입장 · N = 1 |
| 6 | 견습 멤버의 견습 보스 | 싱글 · 파티원이 끌려 들어가지 않는다 |
| 7 | 보상 기준: 견습 멤버 = 견습 스테이지 · 친구 = 자기 스테이지 | 같은 몬스터 처치 로그 두 줄 |
| 8 | 원상 복구 | 고아 보스 0 |

견습 상태를 스탠드인에 어떻게 주는지: Attribute로 읽지 말고 `TutorialState`의 상태 함수가 스탠드인 표를 받도록 **검증 쪽에서** 감싼다(제품 코드에 스탠드인 분기를 넣지 않는다). 안 되면 실제 Player를 견습 중으로 두고 스탠드인을 "견습 아님"으로 둔다.

## 4. 합격 기준

| # | 항목 |
|---|---|
| 1 | 8/8 · 기존 `party selftest` 11항목 · 23-1 견습 검증 회귀 없음 · 에러 0 |
| 2 | 스크린샷: 견습 1단계의 [친구 부르기] · 파티 탭의 같은 버튼 · 직업 선택 문구 · 로딩 문구(찍기 어려우면 클라 로그로 "문구 n번 표시") |
| 3 | `PromptGameInvite`는 Studio에서 창이 안 뜰 수 있다 - 호출이 에러 없이 돌아오는 것까지만 확인하고 "사람이 확인할 것"에 남긴다 |

**진짜 합격 기준**: ① **3번** - 견습 멤버가 낀 파티의 보스 HP가 **들어간 사람 수**로 계산된다. ② **6번** - 견습 보스는 끝까지 싱글이다.

## 5. 하지 않는 것

- 입장 밴드 확대 · 모드 선택 화면 · 커스텀 초대 창. 다중 클라 실기(B 절차에 항목 추가만 - 20.65 [4]에 "견습 중인 친구와 파티 → 같이 슬라임 사냥 → 친구가 보스전에 들어가면 견습자는 남는다" 한 줄).
