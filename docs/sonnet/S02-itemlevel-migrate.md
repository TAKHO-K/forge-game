# S02 — 부풀려진 itemLevel 세이브 값 이관 (비가역)

> 의존성: **S01 완료 · 합격**(드랍 규칙이 검증된 뒤에만 옛 값을 버린다). 규모: 소. 설계 출처: PRD 20.72 [2-7] · [5] 8단계.
> 이 세션은 **되돌릴 수 없는 유일한 단계**다 - 원래 값을 버린다. 미퍼블리시라 개발 계정만 해당된다. 그래서 작게, 따로 한다.

## 0. 한 줄 목표

세이브에 남아 있는 옛 규칙의 아이템(`itemLevel = 레벨 × tier 보너스` - 최대 420급)을 `itemLevel = min(itemLevel, dropStage + 2)`로 자르고, 그 부등식을 전역 불변식으로 만든다.

## 1. 먼저 읽을 것

- PRD: 20.72 [2-5] · [2-7] · S01이 남긴 절
- 코드: `shared/data/SaveConfig.lua`(`saveVersion` - 지금 23) · `server/SaveSystem.lua`의 `migrate` 전체(버전별 분기의 모양 · 직업별 `classState` · 가방 · 착용 장비를 어떻게 도는지) · `isValidProfile` 전체 · 합성 프로필로 이관을 검증한 기존 블록(26-1의 v22 → v23 검증을 grep `v22` · `v23`로 찾는다)
...
- `saveVersion` 23 → **24**.
- `migrate`에 v23 → v24 분기: **가방의 모든 장비 + 모든 직업의 착용 장비(갑옷 · 장갑 · 신발)**에 대해
  `item.itemLevel = math.min(item.itemLevel, item.dropStage + 2)`. `dropStage`가 없는 항목(있다면)은 건드리지 않고 개수를 로그로 남긴다.
- **보석은 건드리지 않는다**(`dropStage`가 없고, 보석 itemLevel은 옵션 계수 f에만 쓰이며 f는 125에서 동결이다 - 20.72 [2-7]).
- 무기 · 강화 단계 · 옵션 · 등급 · `dropStage` · `tierIndex`는 그대로.
- 이관 로그: `[forge-game] v24 이관: 장비 N개 중 M개 itemLevel 절단(최대 a → b)`.

### 단계 2 — 불변식

- `isValidProfile`에 추가: 모든 장비에서 `itemLevel <= dropStage + 2`(둘 다 숫자일 때). 어기면 기존 실패 처리와 같은 길.
- **주의**: S01의 견습 확정 지급(`itemLevel = stage`) · 보스 드랍(`S ~ S + 2`) · 잡몹(`S − 2 ~ S + 2`)이 전부 이 부등식 안인지 S01의 `Loot`를 다시 읽어 확인한다. 부등식을 어기는 정상 경로가 하나라도 있으면 **불변식을 넣지 말고 멈춘다**(미결 보고).

## 3. 자동 검증 블록 — 기존 이관 검증 블록 옆에 (가)

합성 프로필 하나에 아래 아이템을 넣고 `migrate` → `isValidProfile`:

| # | 넣는 것 | 기대 |
|---|---|---|
| 1 | 가방: `dropStage 100 · itemLevel 420` | 102 |
| 2 | 가방: `dropStage 50 · itemLevel 60`(레벨이 스테이지보다 높았던 정상 아이템) | 52 |
| 3 | 가방: `dropStage 50 · itemLevel 52`(경계) · `dropStage 50 · itemLevel 40` | 52 · 40(그대로) |
| 4 | 다른 직업의 착용 갑옷: `dropStage 30 · itemLevel 95` | 32 |
| 5 | 보석: itemLevel 300 | 300(그대로) |
| 6 | 이관 뒤 `isValidProfile` | true |
| 7 | 이관을 **두 번** 돌린다 | 결과 동일(멱등) |
| 8 | v24 프로필에 `itemLevel 60 · dropStage 50`을 손으로 넣고 `isValidProfile` | false |
| 9 | 옵션 · 등급 · `locked` · 강화 단계 | 이관 전후 동일 |

**(나)**: 실제 개발 계정의 프로필이 Play 시작 때 이관되는 로그 한 줄을 확인한다(에러 없이 로드 · 절단 개수가 찍힌다). 재시작한 두 번째 Play에서는 "절단 0개"여야 한다.
...
|---|---|
| 1 | (가) 9/9 |
| 2 | 실제 프로필 로드 성공 · 두 번째 Play에서 절단 0 |
| 3 | 기존 검증 회귀 없음(특히 26-1 이관 검증 · 26-2 41/41) |
| 4 | 서버 에러 · 경고 0 |

**진짜 합격 기준**: **(가) 1 · 7 · 9번** - 420이 102가 되고, 두 번 돌려도 같고, itemLevel 말고는 아무것도 안 바뀐다.

## 5. 하지 않는 것

- 강화 · 재화 필드를 미리 만들지 않는다(S03 ~ S05가 단계마다 하나씩 올린다).
- 보석 · 무기를 건드리지 않는다.
