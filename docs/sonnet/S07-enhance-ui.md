# S07 — 강화 UI: 확률표 5행 · 되는 단계 · 불씨 · 방지권 토글 · 구매 · 구간 진입 확인창
...
| 불씨 | `Gauge`(높이 16) + "54% (실패 시 +6%)" · 100%면 "다음 시도 확정 성공". 값 = Attribute 또는 결과 payload(S03) |
| 방지권 토글 2개 | `Toggle` - 보유 0 · 사용 불가 구간(하락 19강 미만 · 초기화 22강 미만) · 불씨 100%에서는 비활성 + **이유 한 줄**. 켜면 확률표가 즉시 다시 그려진다(막힌 행은 취소선 + "0%", 그만큼 유지에 합산). 토글 상태는 클라가 기억하고 `EnhanceRequest(useDrop, useReset)`에 싣는다 |
...
| 결과 줄 | 성공 = 기존 문구 / 유지 = "실패 - 유지 (+23) · 불씨 +6%" / 하락 = `ember` / 초기화 = `danger` + "12강에서 다시 시작합니다. 불씨은 유지됩니다" / 방지 발동 = "하락 방지권이 막았습니다(남은 1장)" |
...
- **(가)** `EnhanceVerify`에 추가: 0 ~ 24강 × 토글 4조합 × 불씨 가득/아님 = 200칸에서 `getOutcomeTable`의 **합 = 1**(1e-9) · 토글이 쓸 수 없는 단계에서 켜도 서버 판정과 같은 표.
- **Play 1(자동)**: 회귀 없음 · 에러 0 · `[S06][UI] 겹침 0쌍`.
- **Play 2(스크린샷 · 클릭 - MCP 사용)**: `/gg`로 무기 단계를 맞춰 ① +10(유지만 값이 있다) ② +19(1강 하락 14%) ③ +23 토글 off → 하락 on → 둘 다 on(12/81/4/2/1 → 12/87/0/0/1 → 12/88/0/0/0 · 합계 100%) ④ 불씨 100%(100/0/0/0/0 · 토글 회색 + 이유) ⑤ 재료 부족(버튼 비활성 + 이유) ⑥ 19강 첫 시도 확인창 → 취소 / 확인 ⑦ 구매 행(골드 부족 · 충분) ⑧ 환생 탭이 전과 같다 ⑨ 폰 가로 에뮬레이터 1장.
- 실제 시도 3회: 서버 로그의 결과와 화면의 결과 줄 · 불씨 값이 같다.
...
단계 1 ~ 2의 "확률표 · 비용 · 불씨 · 강화 버튼"까지에서 끊을 수 있다(토글 · 구매 · 확인창은 이어서).
