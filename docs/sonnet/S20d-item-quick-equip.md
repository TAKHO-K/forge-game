# S20d — 장비 빠른 장착 + 칸 수 측정 + 보석 자동 장착 미리보기

> 의존성: S20c(보석과 같은 입력 규칙 · 공용 함수 경로 - `GemActions`). 규모: 대(UI + 서버 확인). **S21a · S21보다 먼저 한다.** 순서: S20d → S21a(고스테이지 수치 감사) → S21.
> 이 세션은 **서버의 장비 장착 · 해제 규칙을 바꾸지 않는다.** 입력 방식(PC 더블클릭 · 우클릭 / 폰 탭 선택 → 버튼)과 표현만 바꾼다. 거절 이유 응답이 서버에 없으면 `GemEquipResult`처럼 **이유 코드 응답만** 추가한다.

## S20c 결정 (PRD 20.109에 기록)

- S20c `[x]`. 사람 확인(실제 Esc 홈 먼저 취소 · 진짜 폰 터치 · 토스트 탭 높이 40)은 실기기 묶음으로.
- "자동 장착이 찬 홈의 좋은 옵션 보석을 밀어냄": 밀려난 보석은 보석칸으로 돌아오므로 **규칙은 유지**하고, 자동 장착 직전에 **"교체될 보석: OOO" 미리보기**를 보여준다(PC 호버 툴팁 / 폰 [장착] 버튼 위 한 줄). → 본 작업 D.
- 해제 기능 · 강화대 12stud 제한 · 보석칸 정리 수단은 **Fable 미결 유지**(이 세션에서 손대지 않는다).

## 0. 한 줄 목표

가방 장비를 PC 더블클릭 · 우클릭 / 폰 탭 → [장착]으로 빠르게 장착 · 해제하고(보석과 같은 입력 규칙 · 같은 공용 함수 경로), 연타 경합을 막고, 가방 칸 수를 설계하는 Fable을 위해 현재 칸 수 · 드랍량을 측정해 보고하고, 보석 자동 장착 직전에 교체될 보석을 미리 보여준다.

## 1. 사전 확인 (코드 전에 보고)

서버의 현재 **장비 장착 · 해제 규칙**을 읽고 **코드를 고치기 전에 보고한다.** 이 규칙은 바꾸지 않는다.

- 해제가 가능한가(부위별 · 태초 무기 등급 불변식 등 해제 금지 조건).
- 같은 부위에 장착 = 교체 방식(기존 장비가 가방으로 가는가 · 파괴인가).
- **가방이 가득 찼을 때** 해제 · 교체 처리(거절인가 · 장비 소실 가능성이 있는가).
- 장착 조건(레벨 · 직업 · 부위 · 강화 등).
- 거리 제한 유무(강화대 12stud 등 - 장착에도 있는가).
- 서버의 **거절 이유 응답이 있는가**(없으면 `GemEquipResult`처럼 이유 코드 응답만 추가한다).
- 클라의 현재 장착 요청 경로(가방 UI → RemoteEvent) · 옛 방식(드래그 · 버튼)이 있다면 그 모양.

## 2. 단계

### A. 장비 빠른 장착 (보석과 같은 입력 규칙 · 같은 공용 함수 경로 - 예 `ItemActions.equip` / `unequip`)

- **PC 더블클릭(0.35초) / 우클릭(마우스를 올린 상태)**: 가방 장비 → 해당 부위 장착(찬 부위면 교체) / 장착 중 장비 → 해제.
- **한 번 클릭 = 기존 상세 보기**(깜빡임 없음).
- **모바일**: 탭 선택 → [장착] / [해제] 버튼(터치 44 이상). **더블탭 사용 안 함.**
- 해제 시 가방이 가득 차면 **거절 + 이유 토스트**(장비 소실 금지). 서버가 거절하면 보석과 같은 복귀 연출 + 이유 토스트.
- 장착 불가(레벨 · 직업 등) 장비는 선택 시 **버튼 회색 + 이유 1줄**.

### B. 요청 중 입력 잠금

- 요청을 보낸 뒤 응답이 올 때까지 같은 종류의 장착 · 해제 입력을 잠근다(연타로 인한 index 경합 방지).

### C. 칸 수 측정 (**수정 금지, 보고만**) - Fable에서 기본 칸 수 · 환생당 증가량을 정하기 위한 자료

- 현재 **가방 칸 수** · **보석칸**(상한 없음) 현황 · PC · 폰의 **한 줄 칸 수**.
- 스테이지 **1 · 25 · 100 · 500**에서 **10분당 평균 드랍 수**(장비 · 보석 따로) - 공식 또는 시뮬레이션으로. 가정은 **[추정]** 표시.
- **"자동 판매 없이 15분 · 20분 사냥 시 필요한 칸 수"** 표.

### D. 보석 자동 장착 미리보기 (S20c 결정)

- 자동 장착 직전에 **"교체될 보석: OOO"** 를 보여준다: PC = 호버 툴팁 / 폰 = [장착] 버튼 위 한 줄. 교체가 없으면(빈 홈으로 들어가면) 안 보인다.
- 규칙은 바꾸지 않는다(밀려난 보석은 보석칸으로 돌아온다).

## 3. 검증

- 로컬: 바뀐 · 새 파일 전부 `luau-compile` · `luau-analyze`(`2>&1`) 필수. 순수 판정은 순수 함수로 빼서 스텁 하네스와 자체 점검으로 잰다.
- 자동 검증: **현재 세션 블록 + 건드린 모듈의 옛 블록만** 돌린다(`verify.current` 갈아 끼우기 · 동반 실행은 COMMON §3). **S04(나)는 계속 제외.** **검증 Play 뒤 `VerifyArmedUntil`을 바로 끈다.**
- **실제 입력 확인**: MCP 더블클릭은 **두 클릭을 붙여 보낸다**(지난 교훈). 창을 연 상태에서 실제 클릭. 폰은 가상 화면 강제(`DebugInventoryScreen`)로 800 × 360.

## 4. 합격 기준

| # | 항목 |
|---|---|
| 1 | PC 더블클릭 · 우클릭: 장착 · 해제 · 교체 |
| 2 | 해제 시 가방이 가득 차면 거절 + 이유 토스트(장비 소실 0) |
| 3 | 장착 불가 장비: 선택 시 버튼 회색 + 이유 1줄 |
| 4 | 폰 800 × 360: 탭 → [장착] / [해제] |
| 5 | 서버 거절 복귀 연출 + 이유 토스트 |
| 6 | 실효 글씨 12px · 터치 44 · 창 연 상태 실제 클릭 |
| 7 | 요청 중 입력 잠금(연타해도 index 경합 없음) |
| 8 | 칸 수 측정 보고(수정 0) |
| 9 | 보석 자동 장착 미리보기(PC 호버 · 폰 한 줄) |
| 10 | 서버 규칙 변경 0(사전 확인 보고와 대조) · 스크립트 에러 0 |

**진짜 합격 기준**: ① 모든 입력(더블클릭 · 우클릭 · 폰 버튼)이 같은 공용 함수 · 같은 서버 요청으로 가는가 ② 폰에서 터치만으로 장비를 끼우고 뺄 수 있는가.

## 5. 하지 않는 것

- 서버의 장비 장착 · 해제 · 교체 규칙 변경 · 해제 기능 신설(Fable 미결).
- 강화대 12stud 장착 제한 · 보석칸 정리 수단(합성 · 판매) · 칸 수 값 변경(측정 보고만).
- 새 색 · 새 에셋 · 새 파티클. 폰에서 더블탭 · 길게 누르기.
