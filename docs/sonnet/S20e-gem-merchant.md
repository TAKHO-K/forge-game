# S20e — 보석상인 NPC + 보석 장착 제한 해제

> 의존성: S20d. 규모: 대(UI + 서버 검사). 순서: S20d → **S20e** → S21a(고스테이지 수치 감사) → S21.
> 이 세션은 **변환 · 리롤의 기능 · 서버 규칙 · 비용을 바꾸지 않는다.** 바꾸는 것은 "어디서 하는가"(강화대 12stud → 보석상인 반경)와 "보석 장착 · 교체의 위치 제한 제거"뿐이다.

## 사전 작업 (S20e 본 작업 전 · 커밋 분리 — 이 지시서 이전에 끝냈다: PRD 20.111)

1. 가방 칸 수 서버 전달(클라의 상수 20 읽기 → 서버 스냅샷 `slots` · 값 20 그대로 · 보석칸은 `gemSlots` 자리만 = nil).
2. `/gg fillbag [n]`(Studio 전용 · 등급이 섞인 장비 n개 지급 · 기본 = 가득 찰 만큼) + 가방 가득 참 해제 거절을 실제 마우스로 확인.
3. 가방 정리 흐름 보고(확인만 · 수정 금지): 일괄 분해 · 판매 유무 · 20칸 가득 찬 가방을 비우는 클릭 · 탭 수 → Fable 입력.

## 사용자 결정 (PRD에 기록)

- 보석 장착 · 교체 = **어디서나**(가방 → 보석 탭). 강화대 12stud 장착 제한 제거.
- 보석 변환 · 리롤 = **커뮤니티 센터 보석상인 NPC에서만**(환생 건물 옆).
- 합성 · 자동 리롤은 아직 설계 미확정(Fable) — **이번에 만들지 않고, 빈 탭 · "준비 중" 표시도 넣지 않는다.**

## 1. 사전 확인 (코드 전에 보고)

- 현재 변환 · 리롤 요청의 **서버 조건**(거리 제한 유무 · 위치), 클라 진입 위치(보석 탭 버튼 등).
- `GemServer`의 강화대 12stud 장착 검사 위치(S20c 보고: `GemServer.server.lua:71`)와 S20c에서 넣은 클라 "강화대 근처" 상태 표시 위치.

## 2. 본 작업

### A. 장착 제한 해제

- 서버의 강화대 거리 검사를 **장착 · 교체**에서 제거한다.
- 클라의 강화대 거리 상태 표시 · 버튼 비활성도 제거한다.
- `GemEquipResult`의 `"far"` 이유 코드는 장착에서 더 이상 쓰지 않는다.

### B. 보석상인 NPC

- 커뮤니티 센터 환생 오브젝트 옆에 **임시 모델**(F5에서 교체) + `ProximityPrompt`(PC E · 모바일 탭). 환생 오브젝트와 같은 방식.
- 누르면 **"보석 공방"** 창(station)이 열리고, 보석 탭에 있던 **변환 · 리롤 UI를 여기로 옮긴다**(기능 · 서버 규칙 · 비용은 그대로).
- **서버 검사**: 변환 · 리롤 요청 시점에 플레이어가 NPC 반경 안인지 서버가 직접 확인한다(클라 판정만 믿지 않는다). 밖이면 거절 + 이유 코드.
- 강화 패널 · 스테이지 패널과 station **상호 닫힘 규칙 동일**.

### C. 보석 탭 안내

- 프로필 플래그(예 `hints.gemMerchantUsed`, 기본 false, nil이면 false로 취급). 보석상인에서 변환 · 리롤을 **1회 성공하면 true.**
- false: 보석 탭 상단에 눈에 띄는 안내 줄 "변환 · 리롤은 커뮤니티 센터 보석상인에게서 [위치 안내]".
- true: 작은 회색 한 줄 + `?` 도움말로 축소.
- 보석 탭에 남은 변환 · 리롤 진입점(끼운 보석 선택 시 버튼 등)은 누르면 무반응 대신 **토스트 "보석상인에게서 가능" + [위치 안내]**.
- **[위치 안내]** = 보석상인 위에 10초간 마커(`BillboardGui` 또는 `Beam`) + 방향 표시. 서버 호출 없이 클라만.

### D. 측정 (수정 금지, 보고만)

- 스테이지 1 · 25 · 100 사냥터에서 커뮤니티 센터까지 **걸어서 걸리는 시간**(PC 기본 이동속도).
- 30초를 넘으면 **"마을 귀환" 필요 여부를 Fable 미결**로 적는다.

## 3. 검증

- 현재 세션 블록 + 건드린 모듈의 옛 블록만(S20c 보석 블록 동반 포함). `luau-analyze` 필수. **S04(나)는 계속 제외.** 검증 Play 뒤 `VerifyArmedUntil`을 바로 끈다.
- 폰 800 × 360 · 실효 12px · 터치 44 · **창 연 상태에서 실제 클릭**. 새 ProximityPrompt · 마커 · 토스트는 스크린샷 1회(COMMON §2).
- 새 저장 필드(`hints.gemMerchantUsed`)가 생기면 `SAVE_VERSION`을 올리고 `migrate()`에 처리를 추가한다. 재접속 유지는 `_verify` 저장으로 확인.

## 4. 합격 기준

| # | 항목 |
|---|---|
| 1 | 어디서나 보석 장착 · 교체 성공(강화대 밖 포함) |
| 2 | 보석상인 반경 안 변환 · 리롤 성공 |
| 3 | 반경 밖에서 요청을 직접 보내면 서버 거절(보안 검사) + 이유 코드 |
| 4 | 안내 줄: 플래그 false → 눈에 띄게 · 1회 이용 후 → 축소 · 재접속해도 유지(`_verify` 저장) |
| 5 | 보석 탭 변환 진입점 → 토스트 + 위치 안내 마커 |
| 6 | 폰 800 × 360: 실효 12px · 터치 44 · 창 연 상태 실제 클릭 |
| 7 | station 상호 닫힘(보석 공방 ↔ 강화 · 스테이지) |
| 8 | 걸어서 걸리는 시간 측정 보고(스테이지 1 · 25 · 100) |
| 9 | 변환 · 리롤 서버 규칙 · 비용 변경 0 · 스크립트 에러 0 |

**진짜 합격 기준**: ① 강화대에서 멀리 떨어진 곳에서 보석을 끼우고 교체할 수 있는가 ② 보석상인 반경 밖에서는 클라를 조작해도 변환 · 리롤이 서버에서 거절되는가.

## 5. 하지 않는 것

- 합성 · 자동 리롤 · 보석 판매 · 해제(Fable 미결) · 빈 탭 · "준비 중" 표시.
- 변환 · 리롤의 확률 · 비용 · 변환권 규칙 변경. 마을 귀환 구현(측정만).
- 새 색 · 새 에셋 · 새 파티클(마커는 기존 `UIColors` · 기본 인스턴스만).
