...
- **Play 2(클릭 · 스크린샷)**: "전"과 같은 다섯 장면 + 투표 배너에서 [동의] 클릭이 서버에 닿는 로그 + 요청 2건이 연달아 올 때 대기열(두 번째가 첫 번째 뒤에 뜬다) + 폰 에뮬레이터 축약형(채팅 · 메뉴바 · 조이스틱과 안 겹친다).
...
**진짜 합격 기준**: ① **신호 대조표에 구멍이 없다**(분할 작업의 유일한 위험은 조용히 빠지는 기능이다). ② 요청 배너가 한 번에 하나만 뜬다.
