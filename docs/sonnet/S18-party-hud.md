# S18 — `PartyHud` · 투표 패널 → ML · MR 부품

> 의존성: S16(파티 목록 자리가 메뉴바 옆으로 옮겨진 뒤). 규모: 중. 설계 출처: **PRD 20.81 [D-2](ML · MR) · [D-3] · [D-5](모바일 축약형) · [D-6] 7번** · 20.62 [7] · 20.70.

## 0. 한 줄 목표

`PartyHud.client.lua`(404줄 - 목록 · 토스트 · 투표 패널이 한 파일)를 **목록 / 요청 배너 / 토스트** 셋으로 나누고 kit 부품으로 다시 짠다. 파티의 동작(초대 · 수락 · 투표 · 추방 · 탈퇴)은 서버 · 클라 어느 쪽도 안 바뀐다.

## 1. 먼저 읽을 것

- PRD: 20.62 [7] · 20.70 [1] · [2](투표 패널 규칙 · 10초 · 채팅을 안 가린다) · 20.81 [D-2] · [D-3] · [D-5]
- 코드: `client/PartyHud.client.lua` 전체 - 시작 전에 **이 파일이 듣는 RemoteEvent · Attribute 전부와 그리는 것 전부를 표로** 적는다 · `server/PartyVote.lua` · `server/PartyServer.server.lua`의 클라 통신부(읽기만) · `client/ui/kit/*` · `client/ui/ScreenMap.lua`

## 2. 단계

1. **"전" 스크린샷**: 솔로 · 더미 3명 파티 · 투표 패널 · 초대 토스트 · 경험치 칩(S09).
2. **분리**(동작 변경 0 - 옮기기):
   - `client/hud/PartyList.client.lua` - 목록(`ScreenMap` `ML.partyList`) · 행 = `ListRow`(40) + `Gauge`(10) 체력 줄 · 경험치 칩.
   - `client/hud/RequestBanner.lua`(모듈) + `client/hud/PartyRequests.client.lua` - **요청 배너**: MR 구역 · 한 번에 하나 · 대기열. 제목 · 본문 · 남은 시간 `Gauge` · 수락(primary)/거절(secondary) `Button`. 투표와 파티 초대가 **같은 부품**을 쓴다. 앞으로의 수락/거절 요청(친구 초대 · 거래 등)도 이 배너로 온다.
   - 파티 토스트 → `Toast.push("TC", …)`.
3. **모바일 축약형**(`Theme.isMobile`): 목록 행에서 이름을 빼고 체력 줄 4개 · 폭 96 · 리더 표시는 줄 왼쪽의 작은 ●. 행을 누르면 2초간 이름이 뜬다(hover 아님).
4. 옛 `PartyHud.client.lua`는 **모든 신호가 새 파일로 옮겨졌음을 1단계의 표로 대조한 뒤** 지운다. 다른 파일이 `PartyHud`의 무엇을 참조하는지 grep(S12의 친구 토스트 · S09의 칩)으로 확인하고 같이 옮긴다.
5. 투표 배너의 수치(10초)는 서버가 내려 주는 값 · 또는 `PartyConfig`에서 읽는다(리터럴 금지).

## 3. 검증

- **Play 1(자동)**: `/gg party selftest` 전 항목 · 27-3 (가)(나) 7/7 · S12 항목 회귀 없음 · 에러 0 · 겹침 0쌍.
- **Play 2(클릭 · 스크린샷)**: "전"과 같은 다섯 장면 + 투표 배너에서 [동의] 클릭이 서버에 닿는 로그 + 요청 2건이 연달아 올 때 대기열(두 번째가 첫 번째 뒤에 뜬다) + 폰 에뮬레이터 축약형(채팅 · 메뉴바 · 조이스틱과 안 겹친다) + **폰 가로 844 × 388 · 더미 3명 파티(=4인)에서 파티 목록 · 대시 버튼 · 체력바 스크린샷**(겹침 0을 겹침 검사 로그와 함께).

## 4. 합격 기준

| # | 항목 |
|---|---|
| 1 | 1단계의 신호 대조표에서 빠진 것 0 |
| 2 | 서버 파일 변경 0(`git diff --stat`) |
| 3 | 스크린샷 · 클릭 재현 · 자동 검증 회귀 없음 · 에러 0 |
| 4 | 새 파일 전부 최상위 local ≤ 120 · 400줄 이하 |
| 5 | **폰 가로(844 × 388) · 파티 4인**에서 파티 목록 × 대시 버튼 · 체력바 겹침 0 - 스크린샷 1장(S16 [△] 결정 1 - 지금은 196 × 4행 목록이 대시 버튼과 8 × 54px · 체력바와 약 42 × 17px 겹친다. 메뉴바 · 조이스틱 구역과의 겹침 0은 S16 그대로 유지) |

**진짜 합격 기준**: ① **신호 대조표에 구멍이 없다**(분할 작업의 유일한 위험은 조용히 빠지는 기능이다). ② 요청 배너가 한 번에 하나만 뜬다. ③ **폰 가로 · 파티 4인에서 파티 목록이 대시 버튼 · 체력바와 안 겹친다**(위 5번 - 이 기준이 O가 되면 S16의 [△]도 [x]로 닫는다).

## 5. 하지 않는 것

- 파티 규칙 · 투표 규칙 · 문구 변경. 장비창의 파티 탭(S19). 다중 클라 실기(사용자 몫 - "사람이 확인할 것"에 "실제 2인에서 투표 배너").
